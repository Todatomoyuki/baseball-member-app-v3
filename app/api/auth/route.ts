    ...
    try {
        ...
        if (ok) {
            await db()
                ...
                .bind(Date.now() + 180 * 86400000, await digest(token(req)))
                .run();
            return json({ authenticated: true }, 200, {
                "Set-Cookie": cookie(req, token(req)),
            });
        }
        ...
        return json(
            {
                error: "保存先に接続できません。時間をおいて再試行してください。",
            },
            503,
        );
    }
...
export async function POST(req: Request) {
    ...
    try {
        ...
        await db()
            ...
            .run();
        return json({ authenticated: true }, 200, {
            "Set-Cookie": await newSession(req),
        });
    } catch {
        return json(
            {
                error: "ログインできませんでした。時間をおいて再試行してください。",
            },
            503,
        );
    }
...
export async function PUT(req: Request) {
    ...
    try {
        ...
        const limited = await rateLimit(req);
        if (limited.blocked)
            return json({ error: "15分後に再試行してください。" }, 429);
        ...
        if (
            ...
            return json(
                { error: "新しいパスワードは12〜128文字で入力してください。" },
                400,
            );
        ...
        await db().batch([
            db()
                .prepare(
                    "INSERT INTO auth_config(id,salt,hash) VALUES(1,?,?) ON CONFLICT(id) DO UPDATE SET salt=excluded.salt,hash=excluded.hash",
                )
                .bind(salt, hash),
        ...
        return json({ ok: true }, 200, { "Set-Cookie": await newSession(req) });
    } catch {
