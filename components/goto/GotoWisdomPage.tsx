...
const portraits = [
  {
    src: "/goto/IMG_3564.JPEG",
    position: "53% 27%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_4134.JPEG",
    position: "50% 35%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_1808.JPEG",
    position: "40% 30%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_3165.JPEG",
    position: "50% 20%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_7906.JPEG",
    position: "50% 15%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_8177.JPEG",
    position: "50% 32%",
    width: 1536,
    height: 2048,
  },
  ...
  {
    src: "/goto/IMG_7899.JPEG",
    position: "50% 20%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_1789_Original.JPEG",
    position: "100% 40%",
    width: 4032,
    height: 3024,
  },
  {
    src: "/goto/4A4EF071-F9EB-4248-BAEF-989BF67E4549.jpeg",
    position: "48% 25%",
    width: 1108,
    height: 1477,
  },
  ...
  {
    src: "/goto/ED02A850-2BBE-4DFA-99E4-51F5E6E42584.jpeg",
    position: "50% 40%",
    width: 960,
    height: 1706,
  },
  {
    src: "/goto/IMG_8176.JPEG",
    position: "100% 15%",
    width: 2048,
    height: 1536,
  },
  {
    src: "/goto/IMG_7941.JPEG",
    position: "50% 22%",
    width: 1536,
    height: 2048,
  },
  {
    src: "/goto/IMG_6871.JPEG",
    position: "55% 20%",
    width: 1536,
    height: 2048,
  },
...
export function GotoWisdomPage() {
  ...
  const fallbackPhoto = loadedPhotos.includes(previousPhoto)
    ? previousPhoto
    : (loadedPhotos.at(-1) ?? 0);
  const visiblePhoto = loadedPhotos.includes(requestedPhoto)
    ? requestedPhoto
    : fallbackPhoto;
  ...
  function listen(event: MouseEvent<HTMLButtonElement>) {
    ...
    if (
      ...
      stage.current.scrollIntoView({
        behavior: scene.reducedMotion ? "auto" : "smooth",
        block: "start",
      });
    }
    scene.listen();
  ...
  function movePortrait(event: PointerEvent<HTMLButtonElement>) {
    if (scene.reducedMotion || event.pointerType !== "mouse" || !canListen)
      return;
    const rect = event.currentTarget.getBoundingClientRect();
    stage.current?.style.setProperty(
      "--drift-x",
      `${(event.clientX - rect.left - rect.width / 2) * 0.015}px`,
    );
    stage.current?.style.setProperty(
      "--drift-y",
      `${(event.clientY - rect.top - rect.height / 2) * 0.012}px`,
    );
  }
  ...
  return (
    <main
      ...
      data-reduced-motion={scene.reducedMotion}
    >
      <div className={styles.ambience} aria-hidden="true">
        <div
          className={styles.backgroundPhoto}
          style={{ backgroundImage: `url("${portraits[visiblePhoto].src}")` }}
        />
        ...
        <div className={styles.visual}>
          ...
          >
            <span className={styles.portraitMotion}>
              ...
              <span
                key={`glint-${scene.round}`}
                className={styles.portraitGlint}
                data-active={isReading}
                aria-hidden="true"
              />
            </span>
            {ripple.key > 0 && (
            ...
            <span className={styles.tapGuide} aria-hidden="true">
              <span className={styles.tapIcon}>
                <MoveUpRight size={17} strokeWidth={1} />
              </span>
              <span>
                後藤君に聞く<small>TAP TO LISTEN</small>
              </span>
            </span>
          </button>
          <span className={styles.photoCredit} aria-hidden="true">
            TATSUYA GOTO / {String(visiblePhoto + 1).padStart(2, "0")}
          </span>
        </div>
            ...
            >
              <div className={styles.quoteHeading} aria-hidden="true">
                ...
                <i>
                  <span
                    style={{
                      transform: `scaleX(${scene.visibleLines / Math.max(1, scene.quote.lines.length)})`,
                    }}
                  />
                </i>
              ...
              <blockquote className={styles.quote}>
                {scene.quote.lines.map((line, index) => (
                  <p
                    ...
                    data-visible={index < scene.visibleLines}
                    aria-hidden={index >= scene.visibleLines}
                  >
                    {line}
                  </p>
                ))}
              ...
              >
                <span>— 後藤 竜冶</span>
                <small>TATSUYA GOTO</small>
              </p>
              <div
                className={styles.afterword}
                data-visible={scene.phase === "complete"}
              >
                <button
                  type="button"
                  className={styles.again}
                  onClick={listen}
                  disabled={scene.phase !== "complete"}
                >
                  <span>もう一度、ありがたいお話を聞く</span>
                  <ArrowUpRight
                    size={17}
                    strokeWidth={1.3}
                    aria-hidden="true"
      ...
      <noscript>
        <p className={styles.noScript}>
          後藤君のお話を聞くには、ブラウザーのJavaScriptを有効にしてください。
        </p>
      </noscript>
    </main>
  );
