...
const CONFIRMATION_PAGE_SIZE = 10;
...
function StatsValues({ player, values, onDelete }: { player: Player; values: PlayerStats; onDelete: () => void }) {
  ...
  return (
    <div className="stats-confirm-player">
      ...
      <button type="button" className="stats-delete-button" onClick={onDelete}>削除</button>
    </div>
  );
}
...
export function StatsView({ players }: { players: Player[] }) {
  ...
  const selectedValues = selectedPlayer ? currentPlayers[selectedPlayer.id] ?? emptyPlayerStats() : emptyPlayerStats();
  ...
  const registerAndReset = () => { setSelectedPlayerId(""); setOpenPlate(null); setStatsTab("entry"); };
  const resetEntry = () => {
    if (!selectedPlayer) return;
    stats.edit((current: StatsData) => ({
      ...current,
      games: {
        ...
        [selectedGameKey]: {
          ...(current.games[selectedGameKey] ?? {}),
          [selectedPlayer.id]: emptyPlayerStats(),
        },
      },
    }));
    setOpenPlate(null);
  ...
  return (
    <section className="stats-page">
      ...
      {statsTab === "confirmation" ? (
        <><div className="stats-confirm-list">{registeredGames.length === 0 ? <div className="panel"><p className="stats-empty">まだ成績が登録されていません。</p></div> : visibleRegisteredGames.map(({ key, game }) => <section className="stats-game-group" key={key}><h2>{game.number === 1 ? game.date : `${game.date}・${game.number}試合目`}</h2><div className="panel">{players.filter((player) => stats.data.games[key]?.[player.id]).sort(sortByNumber).map((player) => <StatsValues key={player.id} player={player} values={stats.data.games[key][player.id]} onDelete={() => deleteRegistration(key, player.id)} />)}</div></section>)}</div>{confirmationPageCount > 1 && <nav className="stats-pagination" aria-label="成績登録確認ページ"><button type="button" className="secondary" disabled={currentConfirmationPage === 1} onClick={() => changeConfirmationPage(currentConfirmationPage - 1)}>前へ</button><span>{currentConfirmationPage} / {confirmationPageCount}</span><button type="button" className="secondary" disabled={currentConfirmationPage === confirmationPageCount} onClick={() => changeConfirmationPage(currentConfirmationPage + 1)}>次へ</button></nav>}<div className="stats-actions"><button type="button" className="secondary" onClick={() => setStatsTab("entry")}>成績を追加登録</button></div></>
      ) : (
        <div className="stats-entry-card panel"><div className="stats-game-fields"><div><label htmlFor="stats-game-date">試合日</label><input id="stats-game-date" type="date" value={selectedDate} onChange={(event) => { setSelectedDate(event.target.value); setSelectedPlayerId(""); setOpenPlate(null); }} /></div><div className="stats-game-number-field"><div className="stats-game-number-control"><input aria-label="試合番号" className="stats-game-number-input" id="stats-game-number" type="number" min="1" step="1" value={selectedGameNumber} onChange={(event) => { setSelectedGameNumber(Math.max(1, Number.parseInt(event.target.value, 10) || 1)); setSelectedPlayerId(""); setOpenPlate(null); }} /><span>試合目</span></div></div></div><div className="stats-player-field"><label htmlFor="stats-player">選手を選択</label><select className="stats-player-select" id="stats-player" value={selectedPlayerId} onChange={(event) => { setSelectedPlayerId(event.target.value); setOpenPlate(null); }}><option value="">選手を選択してください</option>{selectablePlayers.map((player) => <option key={player.id} value={player.id}>#{player.number} {player.name}</option>)}</select></div>
          {selectedPlayer ? <><h2 className="stats-section-heading">打席結果</h2><div className="plate-entry-grid">{selectedValues.plateAppearances.map((result, index) => <div className="plate-entry" key={index}><button type="button" className={`plate-square ${result ? "filled" : ""}`} onClick={() => setOpenPlate(openPlate === index ? null : index)}><small>{index + 1}打席目</small><strong>{result ?? "選択"}</strong></button>{openPlate === index && <div className="plate-result-menu">{PLATE_APPEARANCE_RESULTS.map((option) => <button type="button" key={option} onClick={() => updatePlate(index, option)}>{option}</button>)}</div>}<label className="scoring-position-field"><span>得点圏</span><input type="checkbox" aria-label={`${index + 1}打席目の得点圏`} checked={selectedValues.scoringPosition[index] === true} onChange={(event) => updateScoringPosition(index, event.target.checked)} /></label></div>)}<button type="button" className="plate-add-button" onClick={addPlate} aria-label="打席を追加">＋<small>打席追加</small></button></div><h2 className="stats-section-heading">その他の成績</h2><div className="stats-number-grid">{NUMBER_FIELDS.map(([field, label]) => <label key={field} htmlFor={`stat-${field}`}><span>{label}</span><input id={`stat-${field}`} type="number" min="0" step="1" inputMode="numeric" value={selectedValues[field]} onChange={(event) => updateNumber(field, event.target.value)} /></label>)}</div></> : <p className="stats-entry-placeholder">試合日・試合番号・選手を選択すると成績入力欄が表示されます。</p>}<div className="stats-actions"><button type="button" className="primary" disabled={!selectedPlayer} onClick={registerAndReset}>登録する</button><button type="button" className="secondary" disabled={!selectedPlayer} onClick={resetEntry}>入力をリセット</button></div></div>
      )}
    </section>
  );
}
